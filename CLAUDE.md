...
- `#conocimientos`, right after `#ia`, shows the tools the team actually uses (ChatGPT, Gemini, Perplexity, Claude, GitHub, VS Code) as colored monogram badges (`.knowledge-item` / `.knowledge-icon`), not real brand logo artwork — deliberate, to avoid reproducing trademarked logos while still using each brand's associated color.
